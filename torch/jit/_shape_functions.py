...
def conv_transpose2d_input(input: List[int], weight: List[int], bias: Optional[List[int]] = None, stride: List[int] = [1, 1], padding: List[int] = [0, 0], output_padding: List[int] = [0, 0], groups: int = 1, dilation: List[int] = [1, 1]) -> List[int]:
    has_dilation = len(dilation) > 0
    dim = len(input)
    output_size: List[int] = []
    input_batch_size_dim = 0
...
add_shape_compute_mapping("aten::convolution : (Tensor, Tensor, Tensor?, int[], int[], int[], bool, int[], int) -> (Tensor)", conv_forwards)
add_shape_compute_mapping("aten::conv_transpose2d.input : (Tensor, Tensor, Tensor?, int[], int[], int[], int, int[]) -> (Tensor)", conv_transpose2d_input)
